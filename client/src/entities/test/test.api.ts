...
export const testQueryOptions = (slug: string) =>
  queryOptions({
    queryKey: [...TESTS_ROOT_QUERY_KEY, "public", slug],
...
export const myTestQueryOptions = (slug: string) =>
  queryOptions({
    queryKey: [...TESTS_ROOT_QUERY_KEY, "my", slug],

    queryFn: async ({ signal }) => {
      const { data } = await getMyTest(slug, { signal });
      const test = transformTestDtoToTest(data);
